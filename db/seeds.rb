...
response1 = Response.create({ question: question1, text: "The return value is what an invoked method will be equal to. It is often indicated by the keyword return, or in some langauges is implicity done with the last line of a method."})
response2 = Response.create({ question: question2,  text: "A procedure is a set of statements that can be repeated on demand. A function takes inputs and transforms them into an output. A method is a procedure that is stored on an object."})
response3 = Response.create({ question: question3,  text: "A hash is a collection of keys and their values. Values can be accessed by their key. An array is an indexed list of values. Elements can be accessed by their index. Hashes and arrays have different operations that can be performed on them."})
response4 = Response.create({ question: question4,  text: "A hash is a collections of keys and their values. An object is a collection of state and behavior, and is an instance of a class. JavaScript Objects are more accurately described as hashes or dictionaries."})
response5 = Response.create({ question: question5,  text: "
...
response6 = Response.create({ question: question6,  text: "A class is a blueprint for creating objects that share similar state and behavior. An instance is a concrete implementation of a class, also called an object."})
response7 = Response.create({ question: question7,  text: "An instance method belongs to an object, and cannot be called on the class itself. A class method is owned by the class, and cannot be called on any one instance."})
response8 = Response.create({ question: question8,  text: "
...
response9 = Response.create({ question: question9,  text: "A one to many relationship indicates that an entity can relate to many instances of another entity, but each one of them only relates to one instance. In a many to many relationship, each entity can have an arbitrary number of relationships with the other entity."})
response10 = Response.create({ question: question10,  text: "A column in dependent table will store an independent tables's primary key as a foreign key. The database can only store values in the foreign key column that also exist as primary keys in the independent table. For example, if table A has primary keys 1, 3, and 5 and table B has a foreign key column that references table A, then 1, 3, 5 are the only legal values for that column."})
response11 = Response.create({ question: question11,  text: "A single source of truth refers to coincidences of two objects in a many-to-many relationship. When querying an object to find out about its relations, the single source of truth is consulted instead of any of the related entities themselves."})
response12 = Response.create({ question: question12,  text: "An environment.rb file imports all of the libraries and classes that should be available to the program at run time."})
response13 = Response.create({ question: question13,  text: "An object-relational mapper takes object relationships, which are have has-many/belongs-to relationships, and translates them to foreign-key relationships (and vice-versa). This is necessary because object data is naturally nested, and relational data is naturally flat."})
response14 = Response.create({ question: question14,  text: "An API defines a contract where when a certain output or state change will be given if a particular input is provided. Also refers generically to web services, which are APIs that are accessed via HTTP."})
response15 = Response.create({ question: question15,  text: "Semantic HTML refers to using and organizing HTML elements by their meaning and purpose, rather by their intended visual appearance. It became popular after HTML5, which introduced a variety of new semantic elements and repurposed several visual elements."})
response16 = Response.create({ question: question16,  text: "The DOM is the browser's representation of an HTML document and its styles. It provides a way to programatically access and manipulate these. It is internally represented as a tree. It provides an eventing interface for its nodes as well."})
response17 = Response.create({ question: question17,  text: "A client issues an HTTP request to an HTTP server, which fulfills the request with an HTTP response. Every request is expected to have a matching response. If a response is not given within an expected timeframe (the timeout), it is considered failed. Clients can be browsers or computers that are also acting as servers for other requests."})
response18 = Response.create({ question: question18,  text: "The internet is a network of networks. It's technologies and protocols that allow these computers to send messages and data to each other. Components include the TCP/IP protocols, the physical cabling connecting the computers, the IP address system, and DNS. The web is a system of technologies built on top of this, and includes browsers, HTML, CSS, JavaScript."})
response19 = Response.create({ question: question19,  text: "Validation ensures the integrity of supplied data. It can be done: In the browser, via form validation. This helps the user enter data correctly.
...
response20 = Response.create({ question: question20,  text: "An event fires when an action occurs on a DOM node. These are often user-initiated (such as clicks or hovers), but may be system initiated (such as network request completing or the browser finishing loading the DOM). These events can be listened to, and functions can be executed in response."})
response21 = Response.create({ question: question21, text: "When an event fires on a DOM node, it also fires on each of the parents of that DOM node sequentially. Each node can listen for the event and respond in turn, as well as stop the event from continuing to bubble."})
response22 = Response.create({ question: question22, text: "Referential transparency is when an expression is equivalent to what the expression evaluates to. In other words, any valid operation on the result of an expression can be done on the expression itself."})
response23 = Response.create({ question: question23, text: "
...
response24 = Response.create({ question: question24, text: "
...
response25 = Response.create({ question: question25, text: "Authentication is who you are, authorization is what you can do. For example, you may authenticate that a particular user is who they say they are based on their knowledge of a password, biometric identification, or ability to login to a social network you trust, and you may additionally authorize that user to delete any data they want."})
response26 = Response.create({ question: question26, text: "By hashing a user's password upon account creation and storing the hashed password instead of the plain-text one. When a user logins and supplies their original password, you hash that password and compare it to the one you initially stored."})
response27 = Response.create({ question: question27, text: "Turning a complex data structure, such as an object or hash, into a string. This is useful when passing or storing data through something that doesn't have types (such as HTTP or localStorage)."})
response28 = Response.create({ question: question28, text: "A popular way of organizing code that emphasizes separation of concerns.
...
response29 = Response.create({ question: question29, text: "A way of describing how the time or memory needed to execute an algorithm grows with respect to the number of inputs. For example:
...
response30 = Response.create({ question: question30, text: "When an inner scope accesses an outer scope after the outer scope has finished executing. For example, a function inside of a function can hold a reference to a variable that was declared in the outer function after the outer function has completed. The inner function closed over the outer function's scope, or accessed it via closure."})
response31 = Response.create({ question: question31, text: "Cross-origin resource sharing. It's a method for safely allowing browsers to trust resources that come from a different domain that resulted from a script. It works by including an Access-Control-Allow-Origin header on HTTP responses that either explicitly white-lists the domain or implicitly includes it via a pattern."})
response32 = Response.create({ question: question32, text: "PUT is for replacing one resource with another, PATCH is for modifying parts of a resource in-place."})
response33 = Response.create({ question: question33, text: "Don't repeat yourself, or consolidating implementations that are intended to do the same thing."})
